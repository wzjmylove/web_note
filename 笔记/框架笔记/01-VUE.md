...
> 注：
>
> > 1、**就算存在router-link，也需要加入router-view**（即，将router-link看作一个额外的html标签）
> >
> > 2、router-link和router-view一样，必须使用在App.vue文件里面
> >
> > 3、每次切换页面，`<a>`进入的页面就会追加一个类名，其余则去掉这个类名
...
> > 结果：
> >
> > ![Vue_动态路由_例子](F:\学习\前端培训\笔记\image\Vue_动态路由_例子.png)
>
> 
